...
export const getImageSummary = async (link: string, fileType?: string) => {
  ...
  const result = await ai.models.generateContent({
    ...
    contents: [
      ...
      {
        text: `You are an expert document analyst. Your task is to extract, understand, and clean the textual and structural content from the given image. The goal is to produce text that captures all the *meaningful information* for semantic embedding, removing visual clutter or irrelevant layout details.Follow these rules carefully:
        1. Read and interpret **all readable text** in the image, including headings, tables, and annotations.
        2. If the image contains structured data (tables, lists, charts), **convert it to plain text format** with clear labels.
        3. **Preserve key context**, logical relationships, and meanings — not formatting or coordinates.
        4. **Ignore** decorative elements, watermarks, page numbers, or repeated headers/footers.
        5. If the image contains multiple sections, summarize each one clearly under a heading.
        6. For handwritten or unclear text, make your **best guess** but note uncertainty in brackets \`[unclear: word]\`.
        ...
        10. Ensure the JSON is properly formatted and parsable. the fields: title, content, summary, and keywords.
        Output format: ---title: [If the image has a clear title or heading], content:[Cleaned, structured text here], summary (optional):[A 2–3 line summary capturing main topic or insight], keywords:[List of 5–10 relevant keywords]---`,
      },
    ],
  });
  return result?.text || "";
  ...
  const result = await ai.models.generateContent({
    ...
    contents: [
      ...
      },
      {
        ...
        8.Output only the JSON object — no text, no explanation, no code block formatting.
        9.Do not include triple backticks (\`\`\`) or any labels like 'json'.
        10. Ensure the JSON is properly formatted and parsable. 
        the fields: title, content, summary, and keywords.Output format: ---title: [If the video has a clear title or heading], content:[Cleaned, structured text here],
  ...
  const result = await ai.models.generateContent({
    ...
    contents: [
      ...
      },
      {
        ...
        content:[Cleaned, structured text here],
        summary (optional):[A 2-3 line summary capturing main topic or insight], keywords:[List of 5-10 relevant keywords]---`,
      },
    ],
  });
